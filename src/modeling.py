...
from src.file_utils import cached_path
...
TF_WEIGHTS_NAME = 'viz.ckpt'

'''def load_tf_weights_in_bert(model, tf_checkpoint_path):
    """ Load tf checkpoints in a pytorch viz
    """
    try:
        import re
        import numpy as np
    ...
    for name, array in zip(names, arrays):
        ...
        # which are not required for using pretrained viz
    ...
    def __init__(self,
        ...
        Args:
            ...
            attention_probs_dropout_prob: The dropout ratio for the attention
                probabilities.
            max_position_embeddings: The maximum sequence length that this viz might
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2
            ...
            raise ValueError("First argument must be either a vocabulary size (int)"
                             "or the path to a pretrained viz config file (str)")
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.LayerNorm is not snake-cased to stick with TensorFlow viz variable name and be able to load
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        # We "pool" the viz by simply taking the hidden state corresponding
        # to the first token.
...
class BertPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, BertConfig):
            raise ValueError(
                ...
                "To create a viz from a Google pretrained viz use "
                "`viz = {}.from_pretrained(PRETRAINED_MODEL_NAME)`".format(
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
        ...
        """
        Instantiate a BertPreTrainedModel from a pre-trained viz file or a pytorch state dict.
        Download and cache the pre-trained viz file if needed.

        Params:
            pretrained_model_name_or_path: either:
                - a str with the name of a pre-trained viz to load selected in the list of:
                    . `bert-base-uncased`
                    . `bert-large-uncased`
                    . `bert-base-cased`
                    . `bert-large-cased`
                    ...
                    . `bert-base-chinese`
                - a path or url to a pretrained viz archive containing:
                    . `bert_config.json` a configuration file for the viz
                    . `pytorch_model.bin` a PyTorch dump of a BertForPreTraining instance
                - a path or url to a pretrained viz archive containing:
                    . `bert_config.json` a configuration file for the viz
                    . `viz.chkpt` a TensorFlow checkpoint
        ...
        try:
            ...
            logger.error(
                "Model name '{}' was not found in viz name list ({}). "
        ...
        #     logger.info("Weights of {} not initialized from pretrained viz: {}".format(
        #         viz.__class__.__name__, missing_keys))
        # if len(unexpected_keys) > 0:
        #     logger.info("Weights from pretrained viz not used in {}: {}".format(
        #         viz.__class__.__name__, unexpected_keys))
        if len(error_msgs) > 0:
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        #print(extended_visual_attention_mask)
        ...
        #print("size lang_feats: ", lang_feats.size())
        #print("size visn_feats: ", visn_feats.size())
        #print("size pooled_output: ", pooled_output.size())
        #with open('/home/wafa/Documents/Projects/lxmert-master/features.p', 'wb') as f:
            #pickle.dump([lang_feats, visn_feats, pooled_output], f)
