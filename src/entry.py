...
class LXRTEncoder(nn.Module):
    def __init__(self, max_seq_length, mode='x'):
        super().__init__()
    ...
    def forward(self, sents, feats, visual_attention_mask=None):
        ...
        feat_seq, pooled_output = self.model(input_ids, segment_ids, input_mask,
                            visual_feats=feats,
    ...
    def load(self, path):
        ...
        print("Load LXMERT pre-trained viz from %s" % path)
        ...
        # Print out the differences of pre-trained and viz weights.
        load_keys = set(state_dict.keys())
        model_keys = set(self.model.state_dict().keys())
        print()
        print("Weights in loaded but not in viz:")
        for key in sorted(load_keys.difference(model_keys)):
            print(key)
        print()
        print("Weights in viz but not in loaded:")
